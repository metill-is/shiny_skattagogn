...
tiundamork_ui <- function(id) {
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        br(),
        tabPanel("Skipting", plotOutput(NS(id, "dreifing_plot"), height = 700, width = "100%") |> withSpinner()),
        tabPanel("Þróun", plotOutput(NS(id, "magn_plot"), height = 700, width = "100%") |> withSpinner())
      )
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    tiundamork_df <- reactive({
      plot_dat <- tiundamork |>
        ...
        pivot_longer(c(-ar, -tiundarhluti, -fjoldi, -tiundarbreyta)) |>
        group_by(ar, name) |>
        mutate(
          p = value / sum(value),
        ...
        ungroup() |>
        mutate(value_adj = vnv_convert(value, ar))
      ...
      plot_dat |>
        mutate(
          per_pers = plot_value / fjoldi * 1e6,
          text = str_c(
            "Ár: ", ar, "\n",
            "Tíund: ", tiundarhluti, "\n",
          ...
          text = case_when(
            str_detect(name, "[Tt]ekjur|Vaxtagj") ~ str_c(
              text, "\n",
              "Á mann (á mánuði): ", number(per_pers / 12,
                suffix = " kr", accuracy = 1,
                big.mark = ".", decimal.mark = ","
              )
            ),
            TRUE ~ str_c(
              text, "\n",
              "Á mann: ", number(per_pers,
                suffix = " kr", accuracy = 1,
                big.mark = ".", decimal.mark = ","
              )
            )
